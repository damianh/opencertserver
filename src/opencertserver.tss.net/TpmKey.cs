...
public partial class NvPublic
{
    ...
    public byte[] GetName()
    {
        var rawData = GetTpmRepresentation();
        var pubDigest = TpmHash.FromData(nameAlg, rawData);
...
public partial class TpmPublic
{
        ...
        byte[] qualifiedNameOfSigner = null)
    {
        ...
        foreach (var d in expectedPcrValues)
        {
            m.Put(d.buffer, "");
        }

        var expectedPcrHash = TpmHash.FromData(pcrDigestAlg, m.GetBytes());
        ...
        out byte[] encryptedSecret)
    {
        ...
        var cvTpm2B = Marshaller.GetTpmRepresentation(cvx);
        Transform(cvTpm2B);

        var symDef = TssObject.GetSymDef(this);
        var symKey = KDF.KDFa(nameAlg, seed, "STORAGE",
            nameOfKeyToBeActivated, [], symDef.KeyBits);
        Transform(symKey);
        ...
        var hmacKey = KDF.KDFa(nameAlg, seed, "INTEGRITY",
            [], [], hmacKeyBits * 8);
        Transform(hmacKey);
        var outerHmac = CryptoLib.Hmac(nameAlg, hmacKey,
...
public partial class TssObject
{
        ...
        byte[] keyData = null)
    {
        ...
        var seed = Globs.GetRandomBytes(nameSize);
    ...
    {
        var symDef = GetSymDef(parent.Public);
        ...
        var symKey = KDF.KDFa(parent.Public.nameAlg,
            parentSymSeed,
            "STORAGE",
            GetName(),
            [],
        ...
        var newPrivate = KeyWrapper.CreatePrivateFromSensitive(
            symDef,
            symKey,
            iv,
            Sensitive,
        ...
        out byte[] encSecret)
    {
        ...
        {
            var sens = Marshaller.ToTpm2B(Sensitive.GetTpmRepresentation());
            var toHash = Globs.Concatenate(sens, GetName());
            Transform(toHash);
            var innerIntegrity = Marshaller.ToTpm2B(CryptoLib.HashData(
                Public.nameAlg, toHash));
            var innerData = Globs.Concatenate(innerIntegrity, sens);
            Transform(innerData);
        ...
        byte[] seed;
        var symDef = GetSymDef(pubNewParent).Copy();
        ...
        var symKey = KDF.KDFa(pubNewParent.nameAlg, seed, "STORAGE",
            Public.GetName(), [], symDef.KeyBits);
        Transform(symKey);

        byte[] dupSensitive;
        using (var enc2 = SymCipher.Create(symDef, symKey))
        {
            if (enc2 == null)
            {
                return null;
        ...
        var hmacKey = KDF.KDFa(pubNewParent.nameAlg, seed, "INTEGRITY",
            [], [], npNameNumBits);

        var outerDataToHmac = Globs.Concatenate(dupSensitive, Public.GetName());
        Transform(outerDataToHmac);

        var outerHmac = Marshaller.ToTpm2B(CryptoLib.Hmac(pubNewParent.nameAlg,
            hmacKey, outerDataToHmac));
        Transform(outerHmac);

        var dupBlob = Globs.Concatenate(outerHmac, dupSensitive);
        Transform(dupBlob);
        ...
        out IPublicIdUnion publicId)
    {
        ...
        else if (pub.type == TpmAlgId.Symcipher)
        {
            ...
            var keySize = (symDef.KeyBits + 7) / 8;
            if (keyData == null)
            {
    ...
    public bool VerifySignatureOverData(byte[] data, ISignatureUnion sig)
    {
        var digest = CryptoLib.HashData(CryptoLib.SchemeHash(sig), data);
    ...
    public bool VerifySignatureOverHash(byte[] digest, ISignatureUnion sig)
    {
        ...
        {
            var hmacKey = (Sensitive.sensitive as Tpm2bSensitiveData).buffer;
