...
public sealed class TpmPassThroughDevice : Tpm2Device
{
    ...
    {
        var hasPowerCycled = PowerCycledDirtyBit;
...
public sealed class TcpTpmDevice : Tpm2Device
{
    ...
    /// <summary>
    /// Constructor overload that allows specifying the platform port explicitly.
    ...
    public override void Connect()
    {
        ...
        else
        {
            ...
            var endPointVersion = ReadInt(CommandStream);
    ...
    {
        var hostEntry = Dns.GetHostEntryAsync(hostName);
        ...
        throw new Exception($"could not locate hostName: {hostName}");
    }
    ...
    private void ConnectWorker(string hostName, int port, out NetworkStream theStream, out TcpClient theClient)
    {
        ...
        var parsedOk = IPAddress.TryParse(hostName, out simulatorAddress);
        var ipConTask = theClient.ConnectAsync(parsedOk ? simulatorAddress : GetIPAddressFromHost(hostName), port);
        var res = true;
        if (SocketTimeout != -1)
        {
    ...
    public override bool PlatformAvailable()
    {
        return (TpmEndPtInfo & (int)TpmEndPointInfo.PlatformAvailable) != 0;
    }

    public override bool PowerCtlAvailable()
    {
        return PlatformAvailable() &&
            (TpmEndPtInfo & (int)TpmEndPointInfo.NoPowerCtl) == 0;
    }

    public override bool LocalityCtlAvailable()
    {
        return PlatformAvailable() &&
            (TpmEndPtInfo & (int)TpmEndPointInfo.NoLocalityCtl) == 0;
    }

    public override bool NvCtlAvailable()
    {
        return PlatformAvailable() &&
            (TpmEndPtInfo & (int)TpmEndPointInfo.NoNvCtl) == 0;
    }

    public override bool HasRM()
    {
        return _HasRM || (TpmEndPtInfo & (int)TpmEndPointInfo.InRawMode) == 0;
    }

    public override bool ImplementsPhysicalPresence()
    {
        return (TpmEndPtInfo & (int)TpmEndPointInfo.SupportsPP) != 0;
    }
    ...
    public override bool UsesTbs()
    {
        return (TpmEndPtInfo & (int)TpmEndPointInfo.UsesTbs) != 0;
    }
    ...
    public override void DispatchCommand(CommandModifier active, byte[] inBuf, out byte[] outBuf)
    {
        ...
        b.Append([active.ActiveLocality]);
        if (LinuxTrm && OldTrm)
        {
            b.Append([0, 1]);
        }
    ...
    public override bool ActGetSignaled(TpmRh actHandle)
    {
        ...
        var status = ReadInt(PlatformStream);
    ...
    {
        var val = Globs.NetToHost4(Read(stream, 4));
        return val;
    }
    ...
    {
        var bufLen = Globs.NetToHost4(Read(stream, 4));
        var buf = Read(stream, bufLen);
        return buf;
    }
    ...
    private void NotifyWorker(CommsSort sort, NetworkStream stream, byte[] data)
    {
        lock (CallbackLock)
        {
            var c = (stream == PlatformStream) ? Channel.Platform : Channel.Data;
            if (Notifier != null)
            {
