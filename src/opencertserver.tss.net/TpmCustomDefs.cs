...
//        [MarshalAs(1, MarshalType.FixedLengthArray)]
    ...
    {
        var len = CryptoLib.DigestSize(alg);
        var bb = new byte[len];
    ...
    public static TpmHash FromObject(TpmAlgId hashAlg, object data)
    {
        ...
        var temp = Marshaller.GetTpmRepresentation(data);
    ...
    {
        var temp = Marshaller.GetTpmRepresentation(objectToExtend);
...
public partial class TpmHandle
{
    ...
    {
        var runningName = Marshaller.GetTpmRepresentation(hierarchyHandle);
        foreach (var pub in children)
        {
            var thisName = pub.GetName();
...
public partial class PcrSelection
{
    ...
    {
        return new PcrSelection(bank, [index]);
    }
    ...
    {
        var pcrs = new uint[pcrCount == 0 ? MaxPcrs : pcrCount];
        for (var i = 0; i < pcrCount; ++i)
        {
            pcrs[i] = (uint)i;
        }
    ...
    public void SelectPcr(uint pcrNumber)
    {
        ...
        var byteNum = pcrNumber / 8;
        var bitNum = pcrNumber % 8;
...
public class PcrValueCollection : TpmStructureBase
{
    ...
    public PcrValueCollection(PcrSelection[] pcrSelection, Tpm2bDigest[] values)
    {
        ...
        var count = pcrSelection.Sum(sel => sel.GetSelectedPcrs().Length);
        Values = new PcrValue[count];
        // Now set the PcrValue[] based on the selection and values
        count = 0;
        foreach (var sel in pcrSelection)
        {
            foreach (var pcrNum in sel.GetSelectedPcrs())
            {
    ...
    public TpmHash GetSelectionHash(TpmAlgId hashAlg)
    {
        ...
        var selections = GetPcrSelectionArray();
        foreach (var sel in selections)
        {
            var pcrIndices = sel.GetSelectedPcrs();
            foreach (var index in pcrIndices)
            {
                var v = GetSpecificValue(sel.hash, index);
