...
public sealed class Tbs : IDisposable
{
    ...
    {
        var num1 = GetLoadedEntities(Tpm, Ht.Transient).Length;
        var num2 = GetLoadedEntities(Tpm, Ht.LoadedSession).Length;
        var num3 = GetLoadedEntities(Tpm, TpmHelpers.GetEnumerator<Ht>("ActiveSession", "SavedSession")).Length;
    ...
    internal void DispatchCommand(TbsContext caller, CommandModifier active, byte[] inBuf, out byte[] outBuf)
    {
        lock (this)
        {
            ...
            var legalCommand = CommandProcessor.CrackCommand(inBuf,
            ...
            var cc = commandHeader.CommandCode;

            // Lookup command
            var command = Tpm2.CommandInfoFromCommandCode(cc);
            if (command == null)
            {
            ...
            var neededObjects = GetReferencedObjects(caller, inHandles);
            var neededSessions = GetSessions(caller, inSessions);
            var neededEntities =
                neededObjects != null
                    ? neededSessions != null
            ...
            if (neededObjects == null || neededSessions == null)
            {
                ...
                var ret = FormatError(TpmRc.Handle);
                outBuf = ret;
                return;
            }
            ...
            while (true)
            {
                ...
                var res = GetResultCode(responseBuf);
            ...
            {
                outBuf = responseBuf;
                UpdateLastUseCount([neededObjects, neededSessions]);
                return;
            }
    ...
    void ProcessTpmClear(TbsContext caller, SlotType excluded = SlotType.NoSlot)
    {
        ContextManager.ObjectContexts.RemoveAll(ctx => ctx.Owner == caller &&
            ctx.TheSlotType != excluded);
        Debug.Assert(excluded != SlotType.NoSlot || ContextManager.ObjectContexts.Count == 0);
    }
    ...
    private void DebugStateSave()
    {
        ...
        {
            var s3 = NumStateSaves % 4 < 2;
            var doPowerCycle = (NumStateSaves % 2 == 0);
            var message = s3 ? "{S3}" : "{S4}";
            if (!doPowerCycle)
            {
                message = "{S3-abort}";
            }
    ...
    private void StateSaveAndReload(bool startupState, bool doPowerCycle)
    {
        ...
        var startHandles = GetAllLoadedEntities(Tpm);
        ...
        var endHandles = GetAllLoadedEntities(Tpm);
        var match = true;
        if (startHandles.Length == endHandles.Length)
        {
            for (var j = 0; j < startHandles.Length; j++)
            {
    ...
    private void ContextSaveEverything()
    {
        ...
        foreach (var c in ContextManager.ObjectContexts)
        {
    ...
    {
        var entityToEvict = ContextManager.GetEntityToEvict(type, pinnedEntities);
    ...
    {
        var numSavedSession = 0;
        // ReSharper disable once NotAccessedVariable
        var numLoadedSession = 0;
        foreach (var o in ContextManager.ObjectContexts)
        {
    ...
    {
        const uint maxHandles = uint.MaxValue;
        ICapabilitiesUnion h;
        var moreData = tpm.GetCapability(Cap.Handles, ((uint)rangeToQuery) << 24, maxHandles, out h);
        if (moreData != 0)
        {
    ...
    private void CheckConsistency(string message = "")
    {
        ...
        foreach (var o in ContextManager.ObjectContexts)
        {
            ...
            if (o.TheSlotType == SlotType.SessionSlot)
            {
                ...
                {
                    var translatedHandle = new TpmHandle(Ht.LoadedSession, o.Context.savedHandle.GetOffset());
