...
public sealed class AsymCryptoSystem : IDisposable
{
    ...
    public static AsymCryptoSystem CreateFrom(TpmPublic pubKey, TpmPrivate? privKey = null)
    {
        ...
        var keyAlgId = pubKey.type;
        cs._publicParms = pubKey.Copy();
        ...
        SymDefObject? symDef = null)
    {
        ...
        {
            var parms = _rsaProvider.ExportParameters(true);
            var rsaPriv = new Tpm2bPrivateKeyRsa(parms.P);
            sens = new Sensitive([], [], rsaPriv);

            tpmPub = new TpmPublic(nameAlg, keyAttrs, [],
                new RsaParms(symDef,
                    scheme,
        ...
        else if (_ecdsaProvider != null || _ecDhProvider != null)
        {
    ...
    {
        TpmPublic pub;
        var priv = GetPrivate(out pub);
        var m = new Marshaller(priv.buffer);
        var privSize = m.Get<ushort>();
    ...
    public ISignatureUnion SignData(byte[] data, TpmAlgId sigHash)
    {
        ...
        if (rsaParams != null)
        {
            ...
            var sigScheme = rsaParams.scheme.GetUnionSelector();
        ...
        var eccParms = _publicParms.parameters as EccParms;
        if (eccParms != null)
        {
    ...
    {
        return VerifySignature(digest ?? [], true, signature);
    }
    ...
    private bool VerifySignature(byte[] data, bool dataIsDigest, ISignatureUnion signature)
    {
        ...
        var eccParams = _publicParms.parameters as EccParms;
        if (eccParams != null)
        {
    ...
    {
        var rr = new RawRsa(_rsaProvider.ExportParameters(true), _rsaProvider.KeySize);
        var plainText = rr.OaepDecrypt(cipherText, OaepHash, label);
        return plainText;
    }
...
public class RawRsa
{
    ...
    {
        var plainX = FromBigEndian(plain);
        var cipher = BigInteger.ModPow(plainX, E, N);
        var cipherX = ToBigEndian(cipher, KeySize);
        return cipherX;
    }

    public byte[] RawDecrypt(byte[] cipher)
    {
        var cipherX = FromBigEndian(cipher);
        var plain = BigInteger.ModPow(cipherX, D, N);
        var plainX = ToBigEndian(plain, KeySize);
        return plainX;
    }

    public byte[] OaepEncrypt(byte[] data, TpmAlgId hashAlg, byte[] encodingParms)
    {
        var encLen = NumBits / 8;
        var zeroTermEncoding = GetLabel(encodingParms);
        var encoded = CryptoEncoders.OaepEncode(data, zeroTermEncoding, hashAlg, encLen);
        var message = FromBigEndian(encoded);
        var cipher = BigInteger.ModPow(message, E, N);
        var encMessageBigEnd = ToBigEndian(cipher, KeySize);
    ...
    {
        var zeroTermEncoding = GetLabel(encodingParms);
        var cipher = FromBigEndian(cipherText);
        var plain = BigInteger.ModPow(cipher, D, N);
        var encMessage = ToBigEndian(plain, KeySize);
        byte[] message;

        var decodeOk = CryptoEncoders.OaepDecode(encMessage, zeroTermEncoding, hashAlg, out message);
        if (!decodeOk)
        {
    ...
    public byte[] PssSign(byte[] m, TpmAlgId hashAlg)
    {
        ...
        var defaultPssSaltLength = 0; // KeySize - CryptoLib.DigestSize(hashAlg) - 1 - 1;

        // Encode
        var em = CryptoEncoders.PssEncode(m, hashAlg, defaultPssSaltLength, NumBits - 1);
        var message = FromBigEndian(em);

        // Sign
        var sig = BigInteger.ModPow(message, D, N);
        var signature = ToBigEndian(sig, KeySize);
        return signature;
    }
    ...
    public bool PssVerify(byte[] m, byte[] signature, TpmAlgId hashAlg)
    {
        // The TPM uses the maximum salt length
        var defaultPssSaltLength = 0; //  KeySize - CryptoLib.DigestSize(hashAlg) - 1 - 1;
        var sig = FromBigEndian(signature);
        var emx = BigInteger.ModPow(sig, E, N);

        var em = ToBigEndian(emx, KeySize);

        var ok = CryptoEncoders.PssVerify(m, em, defaultPssSaltLength, NumBits - 1, hashAlg);
        return ok;
    }
    ...
    {
        var k = KeySize;
        var em = CryptoEncoders.Pkcs15Encode(m, k, hashAlg);
        var message = FromBigEndian(em);
        var sig = BigInteger.ModPow(message, D, N);
        var signature = ToBigEndian(sig, KeySize);
        return signature;
    ...
    public bool PkcsVerify(byte[] m, byte[] s, TpmAlgId hashAlg)
    {
        ...
        var emDecrypted = ToBigEndian(emx, KeySize);

        var emPrime = CryptoEncoders.Pkcs15Encode(m, k, hashAlg);
...
internal class RawEccKey
{
    ...
    internal static ECCurve GetEccCurve(TpmPublic pub)
    {
        ...
        var signing = pub.objectAttributes.HasFlag(ObjectAttr.Sign);
        var encrypting = pub.objectAttributes.HasFlag(ObjectAttr.Decrypt);
