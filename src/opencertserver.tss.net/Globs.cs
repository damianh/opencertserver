...
public class Globs
{
    ...
    public static int GetRandomInt(int upperBound = int.MaxValue)
    {
        if (upperBound == 0)
        {
            return 0;
        ...
        string linePrefix = "    ", string lineSuffix = "")
    {
        ...
        for (var i = 0; i < buf.Length; i += bytesPerRow)
        {
            res += linePrefix;
    ...
    {
        var maxLen = Math.Min(a1.Length, a2.Length);
        ...
        {
            for (var j = 0; j < a1.Length - a2.Length; j++)
            {
                if (a1[j] != 0)
                {
                    return false;
