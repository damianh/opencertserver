...
public abstract class PolicyAce
{
    ...
    {
        var ss = string.Format("Type = {0}, NodeID = {1}, BranchId = {2}",
...
public class TpmPolicyOr : PolicyAce
{
    ...
    public Tpm2bDigest[] GetPolicyHashArray(TpmAlgId hashAlg)
    {
        ...
        foreach (var branch in PolicyBranches)
        {
    ...
    internal override TpmHash GetPolicyDigest(TpmAlgId hashAlg)
    {
        ...
        foreach (var branch in PolicyBranches)
        {
            var branchPolicyHash = branch.GetPolicyDigest(hashAlg);
    ...
    {
        var branchList = GetPolicyHashArray(policy.PolicyHash.HashAlg);
    ...
    public PolicyAce[][] Branches
    {
        get
        {
            ...
            for (var j = 0; j < PolicyBranches.Count; j++)
            {
...
public class TpmPolicyCounterTimer : PolicyAce
{
    ...
    internal override TpmHash GetPolicyDigest(TpmAlgId hashAlg)
    {
        ...
        var toHash = m.GetBytes();
        var args = CryptoLib.HashData(hashAlg, toHash);
...
public class TpmPolicyNV : PolicyAce
{
    ...
    internal override TpmHash GetPolicyDigest(TpmAlgId hashAlg)
    {
        ...
        m.Put(Offset, "offset");
        m.Put(Operation, "operation");
        var args = CryptoLib.HashData(hashAlg, m.GetBytes());
...
public class TpmPolicySigned : TpmPolicyWithExpiration
{
    ...
    internal override TpmRc Execute(Tpm2 tpm, AuthSession sess, PolicyTree policy)
    {
        var nonceTpm = UseNonceTpm ? Globs.CopyData(sess.NonceTpm) : [];
        ...
        var responseCode = tpm._GetLastResponseCode();
...
public class TpmPolicyAuthorize : PolicyAce
{
        ...
        TpmHandle hSigKey, ISigSchemeUnion scheme = null)
    {
        ...
        var dataToSign = Globs.Concatenate(approvedPolicy, policyRef);
        var aHash = CryptoLib.HashData(pub.nameAlg, dataToSign);
...
public class TpmPolicyNvWritten : PolicyAce
{
    ...
    internal override TpmHash GetPolicyDigest(TpmAlgId hashAlg)
    {
        ...
        var writtenName = IsNvIndexRequiredToHaveBeenWritten ? (byte)1 : (byte)0;
