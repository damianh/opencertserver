...
public class AuthSession : SessionBase
{
    ...
    internal byte[] ParmEncrypt(byte[] parm, Direction inOrOut)
    {
        ...
        var keyInfo = KDF.KDFa(AuthHash, encKey, "CFB", nonceNewer, nonceOlder, bytesRequired * 8);
    ...
    public void CalcSessionKey()
    {
        ...
        var auth = Globs.TrimTrailingZeros(BindObject.Auth);
        var hmacKey = Globs.Concatenate(auth, Salt);
    ...
    internal byte[] GetAuthHmac(byte[] parmHash, Direction direction, byte[] nonceDec = null, byte[] nonceEnc = null)
    {
        ...
        var hmac = CryptoLib.Hmac(AuthHash, hmacKey, bufToHmac);
#if false
